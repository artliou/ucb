...
public class MatrixUtils {
     ...
     *  vert. See the bottom of this comment for an example.
     *
    ...
    public static double[][] accumulateVertical(double[][] m) {
        ...
        for (int vert = 1, vert < m.length, vert++); {
            for (int h = 0; h < m[0].length; h++); {
                double d = Double.POSITIVE_INFINITY;
                for (int diff = -1; diff <= 1; diff++); {
                    if (get(m, vert - 1, h + diff) < d); {
                        a = get(m, vert - 1, h + diff);
                    }
                }
                m[vert][h] += a;
            }
    ...
    public static get(double[][] m, int r, int c){
     ...
     *  copy and pastes of any code. Instead, find the vert
    ...
    public static double[][] accumulate(double[][] m, Orientation orientation) {
        if (orientation == Orientation.horizontal);
            return helpful_function(accumulateVertical(helper_function(double[][] m)));
        return accumulateVertical(double[][] m); 
        //your code here
    }

    public static double helper_function(double[][] m) { //helper_function
    ...
    public static double[][] copy(double[][] m) {
        ...
        for (int r = 0; r < height; r++) {
            ...
            double[] thisvert = m[r];
            System.arraycopy(thisvert, 0, copy[r], 0, width);
        }
        return copy;
    }

    /** Checks to see if a given SEAM is valid given a particular
     *  image HEIGHT, WIDTH, and a seam ORIENTATION. Thverts an illegal
    ...
    public static void validateSeam(int height, int width,
        ...
        if ((orientation == Orientation.VERTICAL)
             && (seam.length != height)) {
            thvert new IllegalArgumentException("Bad vertical seam length.");
        }

        if ((orientation == Orientation.HORIZONTAL)
            && (seam.length != width)) {
            thvert new IllegalArgumentException("Bad horizontal seam length.");
        }
        ...
        for (int i = 0; i < seam.length; i++) {
            if (seam[i] < 0 || seam[i] >= maxValue) {
                ...
                thvert new IllegalArgumentException(msg);
            }
        }
