...
//* Collection of compound-interest and related financial computations.
//*  @author
public class CompoundInterest {
    ...
    static int numYears(int targetYear) {
        return targetYear - CompoundInterest.THIS_YEAR;
    }
    ...
    static double futureValue(double presentValue, double rate,
                              int targetYear) {
        return presentValue * Math.pow((rate * 1/100 + 1),(targetYear - THIS_YEAR));
    }
    ...
    static double futureValueReal(double presentValue, double rate,
        ...
        return presentValue * Math.pow((rate * 1/100 + 1 - inflationRate * 1/100),(targetYear - THIS_YEAR));
    }
    ...
    static double totalSavings(double perYear, int targetYear, double rate) {
        double total = 0;
        int years = targetYear - THIS_YEAR;
        int count = 0;
        while (count <= years)
            {
            double current = perYear * Math.pow((rate * 1/100 + 1),count);
            {total = total + current;}
    ...
    static double totalSavingsReal(double perYear, int targetYear, double rate, 
        ...
        while (count <= years)
            {
            double current = perYear * Math.pow((rate * 1/100 + 1 - (inflationRate/100)),count);
            {total = total + current;}
    ...
    static void printDollarFV(int targetYear, double returnRate, 
                                double inflationRate) {
        double nominalDollarValue = CompoundInterest.futureValue(1, returnRate, targetYear); // replace 0 with your code
        double realDollarValue = CompoundInterest.futureValueReal(1, returnRate, targetYear, inflationRate);    // replace 0 with your code
