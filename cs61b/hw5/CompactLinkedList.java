...
public class CompactLinkedList<T> extends AbstractSequentialList<T> {
    ...
    private class CompactIterator implements ListIterator<T> {
        ...
        @Override
        public T next() {
            return this._next; // REPLACE WITH SOLUTION
        }
        ...
        @Override
        public T previous() {
            return this._prev; // REPLACE WITH SOLUTION
        }
        ...
        public void add(T obj) {
             ...
             * of _size. ########
             * An implementation that allows removals would
            ...
            // FILL IN
            try {
            if (hasNext() == true && this._size = null) {
                for (int i = 0; i < k; i += 1) {
                        obj = previousIndex(); //Create obj before index
                        this._size = this._next; //Move to the next item in list
                    }
                    next(); //MOve to next in the list
                }
            } 
            else {
