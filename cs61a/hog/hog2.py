...
def draw_number(n, dot='*'):
    ...
    """
    "*** YOUR CODE HERE ***"
    #assert 1 <= n and n <= 6, "Dice roll must be between 1 and 6"
    c = f = b = s = False
    if n == 1:
        c = True
    elif n == 2:
...
def play(strategy0, strategy1):
    ...
    # Main Game Execution
    while not (score1 >= goal or score0 >= goal):
        ...
        #switch players
        who = other(who)
    #returns the winner, who is the "other" person because we have passed the turn to
    # the other player, but the original player has actually won the game
    return other(who)
...
def final_strategy_test():
    ...
    ans = compare_strategies(final_strategy)
    print('Win rate:', ans)
    return ans
