...
def analyze_tweet_sentiment(tweet):
    ...
    "*** YOUR CODE HERE ***"
    total_sentiment, words_with_sentiment = 0, 0
    lst = tweet_words(tweet)
    for word in lst:
    ...
    if words_with_sentiment > 0:
        return make_sentiment(total_sentiment/words_with_sentiment)
    else:
...
def find_centroid(polygon):
    ...
    """
    "*** YOUR CODE HERE ***"
    polygon = polygon + [polygon[0]] # add first vertex to end but in a way to not change original list
...
def find_center(polygons):
    ...
    for shape in polygons:
        cx, cy, area = find_centroid(shape)
        total_x += cx * area
        total_y += cy * area
        total_area += area
...
def most_talkative_state(term):
    ...
    state_with_highest_tweet_count = 'n/a'
    max_tweets = -1

    for state in tweets_by_state:
        num_tweets = len(tweets_by_state[state])
        if num_tweets > max_tweets:
            max_tweets = num_tweets
            state_with_highest_tweet_count = state

    return state_with_highest_tweet_count
...
def average_sentiments(tweets_by_state):
    ...
    for state in tweets_by_state:
        ...
        if num_sentiments > 0:
            averaged_state_sentiments[state] = total_sentiment / num_sentiments
...
def group_tweets_by_hour(tweets):
    ...
    for tweet in tweets:
        ...
        if time in tweets_by_hour:
            tweets_by_hour[time].append(tweet)

        else:
            tweets_by_hour[time] = [tweet]
